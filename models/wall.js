var Wall = function(height, startPos) {
  ...
  this.vertices = [startPos.x, startPos.y, startPos.z];
  this.vertices = this.vertices.concat([startPos.x, startPos.y, startPos.z]);
  this.vertices = this.vertices.concat([startPos.x, startPos.y + 1, startPos.z]);
  this.vertices = this.vertices.concat([startPos.x, startPos.y + 1, startPos.z]);
  ...
  this.grow = function(position, corner) {
    if (!corner) {
      this.vertices[this.vertices.length - 9] = position.x;
      this.vertices[this.vertices.length - 7] = position.z;
